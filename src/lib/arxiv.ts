...
const ARXIV_API_URLS = ['https://export.arxiv.org/api/query', 'https://arxiv.org/api/query'] as const;

export async function searchArxiv(query: SearchQuery): Promise<ArxivSearchItem[]> {
  const xml = await fetchArxivFeed(query);
...
async function fetchArxivFeed(query: SearchQuery) {
  ...
  for (const baseUrl of ARXIV_API_URLS) {
    const url = `${baseUrl}?search_query=all:${searchQuery}&start=0&max_results=10&sortBy=${sortBy}&sortOrder=${sortOrder}`;
    ...
    try {
      ...
      lastError = error instanceof Error ? error : new Error('Unknown arXiv request error');
    }
  }
...
function buildArxivPdfUrl(id: string) {
  const normalizedId = id.replace(/^arxiv:/i, '').replace(/v\d+$/i, (match) => match.toLowerCase());
